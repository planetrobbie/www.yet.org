...
Before applying your states to your control node, check that everything looks good by getting information out of your Minion
...
|`cfg01.domainname.`|definition of Salt Master itself, **model repository, timezone, hostname, domainname, IP addresses, repository, Salt accept policy**|
...
Now Prepare the Glusterfs volumes, beware run this only on one controller, ssh in to *ctl01* to run
...
Wait until everything is bootstrapped, in the last command above, make sure the `Schema version` is the same on all nodes and the cluster name is Contrail.
...
Check again and if you still have issues you can remove the following file and restart
