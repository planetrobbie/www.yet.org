...
created_at: 2014-01-04 14:48:00 +0100
kind: article
published: true
tags: ['howto', 'vmware', 'nsx', 'kvm']
---

In this article we will review all the steps required to deploy *[NSX 4.0](http://vmware.com/products/nsx)* a Network Virtualisation offering from *VMware*. We will nest this lab within an ** cloud but you can run the lab anywhere else where you can launch VMs, including *vSphere*, *Virtual Box* or *VMware Fusion*.
...
3. Connect to the Manager WebUI at `http://manager-floatingIP` with `admin/admin` credentials to create a Gateway Node API Entity  			
	...
	g. Click `Save & View` to register the Gateway Node  
...
5. Check Service Node Status, from an SSH connection type
...
4. Now to create the Service Node API Entity we will use a manual process this time for learning purpose. First connect to the Manager WebUI at `http://manager-floatingIP` with `admin/admin` credentials  
	...
	k. Click `Save & View`, to register Service Node  
...
Keep these information with you, you'll need it in the next phase.
...
To provision easily a VM on our Ubuntu 12.04 Hypervisor, we can use a Canonical tool called [vmbuilder](https://help.ubuntu.com/12.04/serverguide/jeos-and-vmbuilder.html).
...
3. Get back to the NSX Manager Dashboard and click on `Add` in the Switches Ports Column and Complete the Wizard with
...
If you have only green lights, like in the screenshot above, you should be able from one VM to ping the other one with great success. Congratulation for reaching this stage of the lab. It was a long way but seeing the ping flowing should give you a least a little joy ;)   

If it's not the case, consider you're lucky, you now have an opportunity to learn more from our Troubleshooting section below.

### Troubleshooting

We are sorry to find you here but when things are broken it's the best time to learn the gory details isn't it ? In this section we'll review some troubleshooting tools, but there are plenty of other ways to investigate, please refer to the Troubleshooting section of the NSX documentation for all the details.   


Please find here some stuff to check.
...
To investigate the Transport Network, NSX Manager provides the Port Connections tool.  
...
Ok we are now done, it was a great adventure for me. I hope you enjoyed it too. NSX for Multihypervisor is really a great software used by lots of customer in production like Rackspace, Paypal, schuberg philis, ATT, NTT. But it's not the only of its kind, VMware also offers a version customised for the vSphere stack which brings many great features: Distributed Firewall, Load Balancing, ...
...
* OpenStack NVP Plugin  [doc](http://docs.openstack.org/trunk/openstack-network/admin/content/nvp_plugin.html)
