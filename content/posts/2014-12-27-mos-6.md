...
* vSAN datastore seems problematic to consume from a *Mirantis OpenStack deployment* for instance ephemeral disks, I'll investigate this issue further. For now I'm only able to consume local datastore but Glance is able to consume vSAN without any issue.
* I also need to troubleshoot VM console access from Horizon dashboard.
