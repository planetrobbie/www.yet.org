...
If you prefer you can instead install your own Chef Server on a brand new Ubuntu OS like this:
...
4. select the version of Chef Server to download
5. transfer the deb package to your node using `scp chef-server_11.0.8-1.ubuntu.12.04_amd64.deb <USERID>@<IPADDRESS>:/tmp/`
6. Install the package: `sudo dpkg -i chef-server.deb`
...
If you get any error message, double check the ability of the chef-server to resolve its hostname.

Before going to the next section, make sure your Chef Server is registered in your DNS server with a fully qualified domain name (FQDN). If necessary change your hostname with:

    sudo hostname 'chef-server.lab.int'
    echo "chef-server.lab.int" | sudo tee /etc/hostname

For internal testing you can also use `/etc/hosts` instead of DNS resolution by adding the following line to it:
...
Before going to the next section, you can play around with *Chef Server Web UI*:
    ...
    https://chef-server.lab.int (replace with your server FQDN)

### Workstation

To interact with your *Chef Server*, you need to configure a *Chef Workstation*.
...
If you have a distro based on *Debian* but with a customized name you'll have to tweak your `/etc/lsb-release` first.
...
This Omnibus installation lives below `/opt/chef`, you'll find the following manifest with all the versions of the installed components:
...
You now have to get an empty Chef repository from Opscode to get you started
...
If you have an upstream repository on github you can connect it with (optional)
...
Every requests comming to the server is authenticated using PKI so you need to copy the following files from your chef-server to your workstation
    ...
    scp /etc/chef-server/admin.pem user@workstation:/home/user/chef-repo/.chef/
    scp /etc/chef-server/chef-validator.pem user@workstation:/home/user/chef-repo/.chef/
...
To create the initial `.chef/.knife.rb` configuration, you first need to add the following line to your `/etc/hosts`

    <CHEF-SERVER-IPADDRESS> chef-server.lab.int
...
chef-validator will be used by new born nodes which will then borrow the identity of chef-validator from the `/etc/chef/validation.pem` private key installed at bootstrap time to register to the server.
...
To use the Light Weight Resource Provider (LWRP) of the magic_shell cookbook inside the newly created one, add the following line to the cookbook `metadata.rb`

    depends 'magic_shell'

You can now the corresponding LWRP in the `aliases/recipes/default.rb` default recipe like this
...
To proceed to this section you first need a base OS installed in a VM. You should normally be able to bootstrap the node using Knife:

    knife bootstrap <VM_IP_ADDRESS> \
      --ssh-user <userid> \
...
For our lab we'll use an offline based bootstrapping instead of downloading the Omnibus package from Opscode, just copy it first
...
Now you can edit it to update the following part like that
    ...
    cat >> /etc/hosts <<'EOP'
    <IPADDRESS>     chef-server chef-server.lab.int
    EOP
...
The first section where we add a line to `/etc/hosts` is necessary because we don't have any DNS resolver that can resolve chef-server.lab.int in this lab.
...
SSH to your node and check if everything looks good by running the following commands.
...
In our previous exercice we had to provision a VM on our own, but Chef can do better then that. Knife offers lots of plugin, like the OpenStack one that can be leveraged to easily provision VMs on OpenStack.

Install it with 

    sudo /opt/chef/embedded/bin/gem install knife-openstack unf
