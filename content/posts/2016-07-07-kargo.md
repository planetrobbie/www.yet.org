...
*[Kargo](https://docs.kubespray.io/)* (a.k.a *Kubespray*) is an initiative to deploy a Kubernetes cluster using Ansible. It will contrast with our previous [Step by Step article](/2016/06/tectonic/) by showing that we can deploy a cluster with a single command, a bit like the newly integrated SwarmKit feature announced in Docker 1.12 `docker swarm init`.
